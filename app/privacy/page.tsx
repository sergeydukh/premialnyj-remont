...
const POLICIES: Record<Locale, Policy> = {
  ru: {
    title: 'Политика конфиденциальности', updated: 'Обновлено: 18 сентября 2026 г.', back: 'На главную',
    ...
    sections: [
      ...
      { title: '5. Аналитика и языковые настройки', paragraphs: ['Для агрегированной статистики посещений используется Vercel Web Analytics. По документации Vercel этот сервис не применяет сторонние cookies, не хранит IP-адрес и не связывает посещения с конкретным человеком. При первом посещении язык определяется по языковым настройкам браузера, без IP-геолокации. Подтверждённый выбор сохраняется в функциональной cookie adelfia-locale и локальном хранилище браузера на срок до одного года.'] },
  ...
  es: {
    title: 'Política de privacidad', updated: 'Actualizada: 18 de septiembre de 2026', back: 'Volver al inicio',
    ...
    sections: [
      ...
      { title: '5. Analítica y preferencias de idioma', paragraphs: ['Utilizamos Vercel Web Analytics para estadísticas agregadas. Según la documentación de Vercel, el servicio no usa cookies de terceros, no guarda la dirección IP ni vincula las visitas con una persona concreta. En la primera visita, el idioma se determina a partir de la configuración del navegador, sin geolocalización por IP. La elección confirmada se guarda durante un máximo de un año en la cookie funcional adelfia-locale y en el almacenamiento local del navegador.'] },
  ...
  en: {
    title: 'Privacy policy', updated: 'Updated: 18 September 2026', back: 'Back to home',
    ...
    sections: [
      ...
      { title: '5. Analytics and language preferences', paragraphs: ['We use Vercel Web Analytics for aggregated traffic statistics. According to Vercel documentation, it uses no third-party cookies, stores no IP address and does not associate visits with an identifiable individual. On a first visit, language is detected from browser settings without IP geolocation. A confirmed choice is stored for up to one year in the functional adelfia-locale cookie and the browser’s local storage.'] },
  ...
  fr: {
    title: 'Politique de confidentialité', updated: 'Mise à jour : 18 septembre 2026', back: 'Retour à l’accueil',
    ...
    sections: [
      ...
      { title: '5. Statistiques et préférences linguistiques', paragraphs: ['Nous utilisons Vercel Web Analytics pour des statistiques agrégées. Selon la documentation de Vercel, ce service n’utilise pas de cookies tiers, ne conserve pas l’adresse IP et n’associe pas les visites à une personne identifiable. Lors de la première visite, la langue est déterminée à partir des réglages du navigateur, sans géolocalisation IP. Le choix confirmé est conservé jusqu’à un an dans le cookie fonctionnel adelfia-locale et le stockage local du navigateur.'] },
